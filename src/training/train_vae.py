...
def test_vae(n_batches:int=2):
    """
    Test rapido del VAE senza salvare nulla.
    Per eseguire: python3 -m src.training.train_vae --test
    """

    print("Test rapido del VAE")
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    set_determinism(seed=42)

    #config minimale per il test
    config_data={
        "patch_size":[64,64,64],
        "batch_size":1,
        "cache_rate":0.0,
    ...
    #caricamento di pochi file per il test
    from src.data.dataset import get_file_list, DATASET_DIRS, create_dataloader
    files=get_file_list(DATASET_DIRS)[:10] #solo 10 file
    ...
    autoencoder, discriminator= setup_models(config_vae, device)
    l1_loss, perceptual_loss, adv_loss= setup_losses(device)
    optimizer_g, optimizer_d= setup_optimizers(autoencoder, discriminator, lr=1e-4)
    scaler_g=GradScaler("cuda")
    scaler_d=GradScaler("cuda")

    #test forward pass su n_batches
    autoencoder.train()
    ...
    print("Test VAE completato")

if __name__=="__main__":
    import argparse
    parser=argparse.ArgumentParser()
    parser.add_argument("--test", action="store_true", help="Esegui test rapido")
